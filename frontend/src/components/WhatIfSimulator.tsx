...
import { Sliders, Clock, AlertTriangle, ShieldCheck, TrendingUp, Zap, Calendar, ArrowRight, BarChart3, Sprout, CheckCircle2 } from "lucide-react";
...
export const WhatIfSimulator: React.FC = () => {
  ...
  // Run Gold-Standard Shapley Biophysical Yield Attribution Engine
  ...
  const getDelaySimulation = (days: number) => {
    ...
    const robi = Math.round((baseAttribution.robiPercent * decayFactor));

    let windowStatus = "OPTIMAL WINDOW OPEN";
    let color = "border-[#00A878] bg-[#DDF7EC]";
    let textColor = "text-[#063B2D]";
    let actionMsg = `Applying Syngenta Stress Buster today for ${selectedField.crop} protects ${Math.round(decayFactor * 100)}% of heat-susceptible yield.`;

    if (days === 1) {
      windowStatus = "WINDOW CLOSING";
      color = "border-amber-400 bg-amber-50/80";
      textColor = "text-amber-900";
      actionMsg = `1-day delay in ${selectedField.crop} spray causes dark respiration sugar loss. Bio-gain drops to +${bioGain} q/ac.`;
    } else if (days === 2) {
      windowStatus = "HIGH RISK DELAY";
      color = "border-orange-400 bg-orange-50/80";
      textColor = "text-orange-950";
      actionMsg = `2-day delay causes partial flower/grain abortion in ${selectedField.crop}. Bio-efficacy drops to ${Math.round(decayFactor * 100)}%.`;
    } else if (days >= 3) {
      windowStatus = "CRITICAL MISSED WINDOW";
      color = "border-rose-400 bg-rose-50/80";
      textColor = "text-rose-950";
      actionMsg = `3+ day delay results in severe yield loss in ${selectedField.crop}. Biological product efficacy drops drastically.`;
    }
  ...
  return (
    <section className="max-w-6xl w-full mx-auto px-4 sm:px-6 py-10 space-y-10 font-sans">
      
      {/* Field Selector Bar */}
      <div className="bg-[#063B2D] text-white p-6 rounded-3xl border border-[#20C98A]/30 shadow-xl space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-3">
          <div className="flex items-center gap-2">
            <Sprout className="h-5 w-5 text-[#20C98A]" />
            <h3 className="font-extrabold text-sm font-display text-white">Active Field Prediction Selector</h3>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs font-mono">
          <div>
            <label className="text-slate-300 block mb-1">Select Field from Portfolio:</label>
            <select
              value={selectedField.id}
              onChange={(e) => handleFieldChange(e.target.value)}
              className="w-full bg-[#10241F] text-emerald-300 border border-slate-700 rounded-xl px-3 py-2.5 font-bold focus:outline-none focus:border-[#20C98A]"
            >
              {savedFields.map((f) => (
          ...
          <div className="bg-[#10241F] p-3 rounded-xl border border-white/10 flex flex-col justify-center">
            <span className="text-slate-400 text-[10px]">CROP & VARIETY</span>
            <span className="font-bold text-white text-sm">{selectedField.crop} ({selectedField.cropVariety || "Standard"})</span>
          </div>

          <div className="bg-[#10241F] p-3 rounded-xl border border-white/10 flex flex-col justify-center">
            <span className="text-slate-400 text-[10px]">FIELD AREA & LOCATION</span>
            <span className="font-bold text-emerald-300 text-sm">{selectedField.areaAcres} Acres ({selectedField.center[0]}°N, {selectedField.center[1]}°E)</span>
          </div>
        </div>
      </div>
      ...
      <div className="text-center max-w-3xl mx-auto space-y-3">
        <h2 className="text-3xl sm:text-4xl font-black font-display text-[#10241F] tracking-tight">
          What if intervention is delayed for {selectedField.name}?
        </h2>
        <p className="text-xs sm:text-base text-slate-600 font-normal leading-relaxed">
          Drag the timeline slider to simulate how delay impacts heat stress loss, biological efficacy, and ROBI return for <strong>{selectedField.crop}</strong>.
        </p>
      </div>
      ...
      <div className="bg-white p-6 sm:p-10 rounded-3xl border border-slate-200 shadow-sm space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-center">
          ...
          <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-1">
            ...
            <span className="text-[10px] text-slate-400 block font-mono">Return on Bio-Investment</span>
          </div>

          <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-1">
            <span className="text-[11px] text-slate-500 font-mono block">MODEL CONFIDENCE</span>
            <span className="text-3xl font-black text-slate-900 font-mono">{baseAttribution.confidenceScore}%</span>
            <span className="text-[10px] text-emerald-600 block font-mono">Open-Meteo Verified</span>
          </div>
        </div>
        ...
        <div className={`p-6 rounded-2xl border ${sim.color} space-y-2`}>
          <div className="flex justify-between items-center flex-wrap gap-2">
            ...
            <span className="text-xs font-mono font-bold text-slate-700">
              Crop: {selectedField.crop} ({selectedField.areaAcres} Acres)
            </span>
          </div>
        ...
        <div className="bg-[#063B2D] text-white p-6 rounded-2xl space-y-4">
          ...
          <div className="space-y-2 font-mono text-xs">
            ...
            <div className="flex justify-between bg-white/5 p-2.5 rounded-xl text-slate-300">
              <span>├── Thermal Stress Impact (GDD Deficit):</span>
              <span className="font-bold text-rose-300">{baseAttribution.thermalDeltaQAc} q/acre</span>
            </div>
            <div className="flex justify-between bg-white/5 p-2.5 rounded-xl text-slate-300">
              <span>├── Soil Moisture Factor ({weather.soilMoistureEst}% Index):</span>
