"""
Benchmark Model 1 across 20 diverse registered farmers in AASRA database
"""
import math
import datetime
import numpy as np
...
# Test benchmark with model bundle
bundle_path = "ps02-engine/data/model1_climate_stress.joblib"
champion_model = joblib.load(bundle_path)
...
for fid, f in AASRA_FARM_DATABASE.items():
    ...
    # Representative weather conditions matching local climate geography
    if "bhuj" in fid or "bikaner" in fid:
        ...
        # Marathwada drought & soil moisture depletion
        t_max = 33.5
        t_min = 21.5
        rh = 36.0
        vpd = 2.9
        ...
        # Floodplain / Excess monsoon saturation
        t_max = 28.0
        t_min = 22.0
        rh = 92.0
        vpd = 0.45
        ...
        # North India winter cold wave / radiation frost
        t_max = 14.5
        t_min = 1.0
        rh = 78.0
        vpd = 0.45
        ...
        # Optimal balanced agro-climatic conditions (Bhopal, Indore, etc.)
        t_max = 26.5
        t_min = 18.0
        rh = 65.0
        vpd = 1.2
    ...
    results.append({
        ...
        "TMin": t_min,
        "SM%": sm,
        "PredictedStress": classes[pred_idx],
        "Confidence": f"{conf*100:.1f}%"
    })

res_df = pd.DataFrame(results)
print("=" * 115)
print("AASRA 20-FARMER DATABASE STRESS CLASSIFICATION BENCHMARK")
print("=" * 115)
